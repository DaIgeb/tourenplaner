...
class Tour extends React.Component<ITourProps, IState> {
    ...
    componentWillReceiveProps(nextProps:ITourProps, nextContext:any) {
    ...
    private loadRestaurant = (newProps:ITourProps):IState => {
        ...
        if (stateUpdateNeeded) {
            ...
            } else if (newProps.tours) {
                state.loading = false;
                let restaurant = newProps.tours.find(r => r.id === this.Id);
                if (restaurant) {
                    state.invalidId = false;
...
export default connectToStores(Tour);